...
void ruyi_mpsc_list_push(ruyi_mpsc_list_t* list, void* pval)
{
	...
	while(true) { /* no ABA */
		...
		if(t == (ruyi_mpsc_list_node_t*)atomic_load_explicit(&list->tail, memory_order_acquire)) {
			...
			if(nt == NULL) {
				...
				while (!atomic_compare_exchange_weak_explicit(&list->tail, (_Atomic ruyi_mpsc_list_node_t**)&t, (_Atomic ruyi_mpsc_list_node_t*)nt, memory_order_release, memory_order_relaxed)) {}
			}
		}
	}
